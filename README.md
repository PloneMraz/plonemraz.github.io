...
Every document below is a working paper or preprint. None is peer-reviewed. Comments
are welcome.
...
The seven numbered papers are not seven theories of seven separate things. They are
seven articulations of a single operation — the operation by which configurations come
into being and hold, and by which that operation becomes articulable from within reality
by the cognizing entities the same operation produces. Each paper asks one question of
it and disclaims what the others answer.
...
**A Programme Overview** — the architecture connecting the seven papers, and the
...
Neither carries a programme number. The first reports the ground the components were
cut from; the second instructs rather than describes.
...
DIL treats an agent in a purely informational environment; RSIL is its embodied
counterpart, in a physical world with a body. Where they coincide it is because the
relational structure coincides; where they diverge, the divergence is deliberate and
marked in place.
...
benchmarking or measured performance.
...
The site is static, hosted on GitHub Pages. Anything pushed to `main` deploys
automatically, usually within a minute.

## Citing

Cite by DOI rather than by this repository — the DOI is the stable identifier, and the
figshare record carries the version history. The PDFs here are mirrors kept for direct
reading.

## License

[MIT](LICENSE) for the code in this repository. The papers themselves are distributed
under the terms recorded on their respective figshare records.
